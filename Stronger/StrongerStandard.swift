...
actor StrongerStandard: Standard, EnvironmentAccessible, HealthKitConstraint, OnboardingConstraint, AccountStorageConstraint {
    enum StrongerStandardError: Error {
    ...
    private let logger = Logger(subsystem: "Stronger", category: "Standard")
    
    
    private var userDocumentReference: DocumentReference {
        get async throws {
            guard let details = await account.details else {
                throw StrongerStandardError.userNotAuthenticatedYet
            }
    ...
    init() {
        if !FeatureFlags.disableFirebase {
            _accountStorage = Dependency(wrappedValue: FirestoreAccountStorage(storeIn: StrongerStandard.userCollection))
        }
    }
